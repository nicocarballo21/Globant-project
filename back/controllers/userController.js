...
module.exports = {
  ...
  setMentee: async (req, res, next) => {
    ...
    if (!updatedMentee || !updatedMentor)
  ...
  postUserObjectives: async (req, res, next) => {
    try {
      ...
      const createdObjective = await postObjectivesToUser(
        user,
        description,
        state,
        due
  ...
  deleteUserObjectives: async (req, res, next) => {
    try {
      ...
      if (!menteeId || !objectiveId)
        return res.status(400).send("Invalid request body.");
      const user = await findUserById(menteeId);
      const objectivePromises = await deleteObjectivesFromUser(
        objectiveId,
        user
      );
      if (!objectivePromises)
        return res.status(404).send("Objective not found.");
  ...
  deleteUserNotes: async (req, res, next) => {
    try {
      ...
      if (!savedMenteeAndDeletedNote)
        return res.status(404).send("The note was not found!.");
      res.sendStatus(204);
    } catch (error) {
      next(error);
    }
